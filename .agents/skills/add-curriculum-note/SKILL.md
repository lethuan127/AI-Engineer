...
This skill is the single source of truth for scaffolding a new numbered note in this repo. Both `.cursor/commands/new-note.md` and `.Codex/commands/new-note.md` delegate here.
...
Read at least one existing note in the same track. Fall back to `7. AI System Architecture/7.1. Codex Platform Managed Agent.md` if the track has no notes yet. Match its structure (heading numbering, table density, callouts).
